...
lines = open('c:/users/ted/VSCode/AoC2021/Day18/sample.txt', 'r').readlines()
...
for i in range(len(lines)-1):
    for j in range(i+1,len(lines)):
        tree1 = Node()
        tree1.process(lines[i].strip())
        tree1.PrintTree()
        tree2 = Node()
        tree2.process(lines[j].strip())
        tree2.PrintTree()
        tree1.snailfish_add(tree2)
        tree1.PrintTree()
        magnitude1 = tree1.find_magnitude()
        ...
        tree2 = Node()
        tree2.process(lines[j].strip())
        tree2.snailfish_add(tree1)
        magnitude2 = tree2.find_magnitude()

        max_magnitude = max(max_magnitude,magnitude1)
        max_magnitude = max(max_magnitude,magnitude2)   

        print(i,j,":",magnitude1,magnitude2,max_magnitude)
